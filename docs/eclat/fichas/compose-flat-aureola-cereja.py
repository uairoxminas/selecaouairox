"""
Monta a pagina final do flat do Aureola colorway canelado cereja (peca 003
da Nuance) a partir de duas imagens ja prontas
(tec_frente_aureola_cereja_flat_raw.png / tec_costas_aureola_cereja_flat_raw.png).

Mesmo metodo das pecas anteriores: croqui plano gerado a partir do render
fotografico aprovado (construcao) e do flat do colorway bronze (estilo).
"""
from PIL import Image, ImageDraw, ImageFont, ImageChops

FRENTE_RAW = "tec_frente_aureola_cereja_flat_raw.png"
COSTAS_RAW = "tec_costas_aureola_cereja_flat_raw.png"
SWATCH = "amostra-canelado.jpg"
OUT = "tec_aureola_cereja_flat.png"
...
def main():
    ...
    centered_text(draw, W / 2, 36, "AUREOLA — CONJUNTO TOP + SHORT, CANELADO CEREJA", f_title, TITLE_COL)
    centered_text(
        draw, W / 2, 96,
        "COLECAO NUANCE · PECA 003 · COLORWAY 2   ·   SEM ESCALA",
        f_sub, GREY,
    )
    ...
    centered_text(draw, sx + sw_box / 2, sy + sw_box + 26, "CANELADO CEREJA", f_note, INK)
    centered_text(draw, sx + sw_box / 2, sy + sw_box + 56, "Mesmo tecido do Eclipse —", f_small, GREY)
    centered_text(draw, sx + sw_box / 2, sy + sw_box + 80, "cor a confirmar cartao Marazul", f_small, GREY)

    ny = y_img + target_h + 40
    draw.line([(80, ny - 20), (W - 80, ny - 20)], fill=(216, 208, 200), width=2)
    notes = [
        ("Mesma modelagem do colorway bronze, sem alteracao — apenas tecido e cor mudam", INK),
        ("Top: alcas finas cruzadas em X na frente, retas (sem cruzar) nas costas — costas abertas, sem escada", INK),
        ("Elastico na base do top, relevo jacquard tom sobre tom com o monograma ECLAT (nao impresso)", MARK),
        ...
        ("Monograma tom sobre tom (no entrelacado) na barra de uma perna do short, ~3cm", MARK),
    ]
    yy = ny
